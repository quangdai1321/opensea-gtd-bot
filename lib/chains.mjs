...
export const CHAINS = {
  ...
  robinhood: { chainId: 4663, coin: 'ETH', explorer: 'https://robinhoodchain.blockscout.com', rpcs: ['https://rpc.mainnet.chain.robinhood.com', 'https://robinhood.drpc.org'] },
...
export async function broadcast(ctx, raw) {
  const hash = ethers.keccak256(raw);
  const res = await Promise.all(
    ctx.providers.map((p) => p.send('eth_sendRawTransaction', [raw]).then(() => null, (e) => e)),
  );
 ...
 */
export async function planGas(ctx, { bump = 2, baseMult = 20n, gasLimit, value, balance }) {
  const [blk, fee] = await Promise.all([ctx.main.getBlock('latest'), ctx.main.getFeeData()]);
