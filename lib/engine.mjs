...
// Doc chung cho moi vi cua cung 1 drop, giu 5s: du de ca loat vi dung chung,
// va van doc lai truoc gio mo neu du an doi cau hinh sat gio.
const sharedReads = new Map();

function sharedRead(ctx, nft) {
  const key = `${ctx.name}:${nft}`;
  if (!sharedReads.has(key)) {
    const p = Promise.all([
      readPublicDrop(ctx.main, nft),
      pickFeeRecipient(ctx.main, nft),
      ctx.main.getBlock('latest'),
      gasMarket(ctx),
    ]).then(([drop, feeRecipient, blk, market]) => ({ drop, feeRecipient, blk, market }));
    sharedReads.set(key, p);
    p.catch(() => {}).finally(() => setTimeout(() => sharedReads.delete(key), 5000));
  }
...
export async function mintPublic(ctx, { w, nft, qty, settings, dry, onEvent, shouldStop }) {
  ...
  // ---- chuan bi ----
  // drop, fee recipient, block moi nhat GIONG NHAU cho moi vi -> doc 1 lan, 6 vi dung chung.
  // Truoc day 6 vi x 6 lenh = 36 lenh cung luc, du de dinh 429 cua RPC cong khai.
  const [shared, stats, balance, nonce] = await Promise.all([
    sharedRead(ctx, nft),
    readMintStats(ctx.main, nft, addr),
    ctx.main.getBalance(addr),
    ctx.main.getTransactionCount(addr, 'pending'),
  ]);
  const { drop, feeRecipient, blk, market } = shared;
  ...
  for (;;) {
    try {
      gas = await planGas(ctx, { bump: settings.gasBump ?? 2, gasLimit, value, balance: balance / BigInt(burst), market });
      break;
    } catch (e) {
      if (burst > 1) {
        burst--;
