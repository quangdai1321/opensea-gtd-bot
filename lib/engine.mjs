...
const BURST_BEFORE_MS = 120;   // ban phat dau tien som hon khoanh khac mo bao nhieu
const BURST_SPACING_MS = 90;   // khoang cach giua cac phat
...
export async function mintPublic(ctx, { w, nft, qty, settings, dry, onEvent, shouldStop }) {
  ...
  const burst = Math.max(1, Math.min(Number(settings.burst || 1), 5));
  let gas;
  try {
