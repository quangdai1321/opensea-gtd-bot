...
async function sendAndWait(ctx, raw, { onEvent, label, openAt }) {
  ...
  return { status: 'done', hash, note: `block ${rc.blockNumber}, phí ${fmt(rc.gasUsed * (rc.gasPrice ?? 0n))}${speed ? `\n   ⏱ bắn${speed}` : ''}` };
}
...
export async function mintPublic(ctx, { w, nft, qty, settings, dry, onEvent, shouldStop }) {
  ...
  // ---- ban ----
  try {
    return await sendAndWait(ctx, raw, { onEvent, label, openAt });
  } catch (e) {
