...
export async function analyze(ctx, nft, { onProgress, topN = 5 } = {}) {
  ...
  if (out.drop && out.drop.startTime > now && supply != null && max != null) {
    ...
    const recent = cut < 0 ? 0 : mints.filter((m) => m.block >= blocks[cut]).length;
    const perMin = recent / RATE_WINDOW_MIN;
    const minsToOpen = (out.drop.startTime - now) / 60;
    const left = Number(max - supply);
    out.upcoming = {
      left, minsToOpen, windowMin: RATE_WINDOW_MIN, recent, perMin,
