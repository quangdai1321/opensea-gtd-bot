 ...
 *   OPENSEA_JWT=eyJ...        (gia tri cookie access_token, F12 > Application > Cookies > opensea.io)
...
export async function fetchEligibility(slug, address) {
  ...
  // Trang web gui token qua cookie access_token (da thu: header authorization bi bo qua)
  if (process.env.OPENSEA_JWT) headers.cookie = `access_token=${process.env.OPENSEA_JWT.replace(/^Bearer\s+/i, '')}`;
